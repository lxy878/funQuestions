...
class Matrix {
  ...
  scale(factor) {
    this.storage = this.storage.map(r=> r.map(n=> n*factor))
  }

  fill(val) {
    this.storage = this.storage.map(r=> r.map(n=> n=val))
  }

  flatten() {
    const result = []
    this.storage.forEach(r=>r.forEach(n=> result.push(n)))
    return result;
  }

  slice(rowRange, colRange) {
    let r0 = rowRange[0], r1 = rowRange[1], c0 = colRange[0], c1 = colRange[1]
    if(r1-r0>this.storage.length){
      r0 = 0
      r1 = this.storage.length
    }
    if(c1-c0>this.storage[0].length){
      c0 = 0
      c1 = this.storage[0].length
    }
    const newMatrix = new Matrix(r1, c1)
    newMatrix.initialize(this.storage.slice(r0, r1).map(row=> row.slice(c0, c1)))
    return newMatrix
  }

  transpose() {
    const rl = this.storage.length, cl = this.storage[0].length
    const newMatrix = new Matrix(cl, rl)
    for(let i=0; i<this.storage[0].length; i++){
      for(let j=0; j<this.storage.length; j++){
        newMatrix.insert(i, j, this.storage[j][i])
      }
    }
