...
function numberOfOnes(arr) {
  let b=0, e=arr.length-1
  let firstOne = -1
  while(b<e){
    let mid = b + Math.floor(((e-b)/2))
    if(arr[mid] === 1){
      if(arr[mid-1]===0){
        firstOne = mid-1
        break
  ...
  let ones = arr.length-firstOne
  if(firstOne===-1){
    ones = arr[0]===0? 0:arr.length
  }
  return ones
}
...
function closestValue(arr, target) {
  ...
  let minDiff = Math.abs(arr[b]-target)
  let min = arr[b]
  if(minDiff > Math.abs(arr[b+1]-target)){
    minDiff = Math.abs(arr[b+1]-target)
    min = arr[b+1]
  }
  if(minDiff >= Math.abs(arr[b-1]-target)){
    min = arr[b-1]
...
function greaterValues(arr, target) {
  ...
  return (b===arr.length-1)? 0:arr.length-b
}
